...
function CourseInstructor({ selcourse }) {
  var columns = [
  ...
  const refresh = () => {
    // update data
    api
      .get('/courseInstructors?course_id=' + selcourse)
      .then((res) => {
        setData(res.data)
      })
      .catch((error) => {
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    ...
    if (errorList.length < 1) {
      api
        .put('/courseInstructors/' + newData.id + '/', newData)
        .then((res) => {
          refresh()
          resolve()
          setIserror(false)
  ...
  const handleRowAdd = (newData, resolve) => {
    ...
    if (errorList.length < 1) {
      //no error
      api
        .post('/courseInstructors/', instructorData)
        .then((res) => {
          refresh()
          resolve()
          setErrorMessages([])
  ...
  const handleRowDelete = (oldData, resolve) => {
    api
      .delete('/courseInstructors/' + oldData.id + '/')
      .then((res) => {
        refresh()
        resolve()
      })
